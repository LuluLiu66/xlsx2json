...
function parseSheet(sheet, setting) {
  ...
  for (let i_row = headIndex; i_row < sheet.data.length; i_row++) {
    ...
    if (setting.type === SheetType.MASTER) {
      ...
      result = Object.assign(result, parsed_row);
    }
  }
...
function parseRow(row, rowIndex, head) {
  ...
  for (let index = 1; index < head.length; index++) {
    let cell = (row[index] || '').trim()

    let name = (head[index].name || '').trim();
    let type = (head[index].type || '').trim();
    ...
    switch (type) {
      ...
      case DataType.UNKNOWN: // number string boolean
        if (isNumber(cell)) {
          result[key][name] = Number(cell);
        } else if (isBoolean(cell)) {
          result[key][name] = toBoolean(cell);
        } else {
          result[key][name] = cell;
        }
        break;
      case DataType.DATE:
        if (isNumber(cell)) {
          //xlsx's bug!!!
          result[key][name] = numdate(cell);
        } else {
          result[key][name] = cell.toString();
        }
        break;
      case DataType.STRING:
        if (cell.toString().startsWith('"')) {
          result[key][name] = parseJsonObject(cell);
        } else {
          result[key][name] = cell.toString();
        }
        break;
